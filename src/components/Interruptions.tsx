...
const Interruptions: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        await updateDoc(doc(db, 'interruptions', editingId), {
  ...
  const handleEdit = (item: Interruption) => {
          ...
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm sticky top-8">
            <div className="flex items-center gap-3 mb-8">
              ...
              <h3 className="text-lg font-black text-slate-900 tracking-tight">
                {editingId ? 'Editar Intercorrência' : 'Nova Intercorrência'}
              </h3>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              >
                {submitting ? (
                  ...
                  <>
                    ...
                    {editingId ? 'Salvar Alterações' : 'Salvar Intercorrência'}
                  </>
                )}
              </button>
              ...
              {editingId && (
                <button
                  ...
                  className="w-full py-3 rounded-xl font-bold text-xs text-slate-400 hover:text-slate-600 transition-colors"
                >
                  Cancelar Edição
                </button>
              )}
